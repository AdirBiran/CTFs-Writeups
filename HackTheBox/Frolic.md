...
another endpoint to check: /playsms
...
decoding the cipher with this website: https://www.dcode.fr/ook-language
<p align="center">
...
searching google for file types with the bytes 50 4B 03 04 indicates its a zip file.
...
we get whats seems to be a password, "idkwhatispass"
...
inside .binary, we have an executable file named rop, owned by root with a sticky bit.
<p align="center">
...
giving the program a long string, will cause in a segmantation fault.
<p align="center">
...
lets find the eip by creating a large non repeating string which will lead to a segmentation fault.
...
we get the address 0x42424242 which confirms the buffer overflow.

the buffer overflow in this case is "Return-to-libc", we will use functions from libc library.
first, we need to find the address of libc by using the command: ldd rop
<p align="center">
...
last thing we need is find the offset to /bin/sh using the command:
...
our final payload is :Buffer + Address_of_System + Address_of_Exit + Address_of_/bin/sh
